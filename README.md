...
1. Edit `/usr/local/etc/rtl_433/rtl_433.conf`:
   	1. If your regional ISM band is not 433.92MHz, set the correct frequency in the "frequency" entry.
	1. Under `## Analyze/Debug options`, comment out stats reporting: `#report_meta stats`
	1. Under `## Data output options`/`# as command line option:` add `output mqtt` and `output json:/var/log/rtl_433/rtl_433.j
son`.  The former has the program publish received packets via `mqtt` and the latter logs received packets to a log file in case you want to do subsequent analysis of devices in your neighborhood.  More options for `mqtt` publishing service are available, but this will get you started.
...
1. Edit the python files to install the hostname of the monitoring system on your local network: `sed s/\<mymonitor\>/xxxx/mqTest.py.template > mqTest.py` and `sed s/\<mymonitor\>/xxxx/ DT.py.template > DT.py`, where "xxxx" is the hostname of your local monitor,  to replace the placeholder "<mymonitor>" with the name of your local host, "xxxx".  (Or just edit the files). If you secured your mqtt publishing, you'll need to provide the username and password for that service in those Python files..
